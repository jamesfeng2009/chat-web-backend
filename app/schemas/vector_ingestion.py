...
class VectorIngestItem(BaseModel):
    ...
    section_level: int | None = Field(None, description="章节层级")

    # 条款级字段
    clause_id: str | None = Field(None, description="条款ID")
    clause_title: str | None = Field(None, description="条款标题")
    clause_order_index: int | None = Field(None, description="条款顺序")

    # 子项级字段
    item_id: str | None = Field(None, description="子项ID")
    parent_item_id: str | None = Field(None, description="父子项ID")
    item_order_index: int | None = Field(None, description="子项顺序")
    ...
    nc_type: str | None = Field(None, description="内容类型")
    score: int | None = Field(None, description="条款分数: 0-4")

    # 文本与定位
    content: str = Field(..., description="向量化源文本")
    loc: dict[str, Any] | None = Field(None, description="定位信息")
    biz_tags: dict[str, Any] | None = Field(None, description="业务标签")

    # 向量（可选）
    embedding: list[float] | None = Field(None, description="预计算的向量，可为null由服务端计算")
